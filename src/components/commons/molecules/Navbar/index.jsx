...
const Navbar = () => {
  ...
  return (
    <>
      <header className="absolute top-4 right-4 z-50 mt-2 flex h-12 w-(--my-width-nav) items-center justify-between bg-transparent px-4">
        <section className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-6 rounded-full bg-(--bg-primary-cray)">
            <p className="font-barlow ml-3 text-sm font-semibold text-black">
              Menu
            </p>
            <button
      ...
      <>
        <div
          className={`fixed top-0 left-0 mx-auto flex h-screen w-full items-center justify-between bg-black/50 px-4 ${isOpen ? "z-50 overflow-hidden opacity-100" : "-z-1 opacity-0"}`}
          onClick={() => setIsOpen(!isOpen)}
        ></div>
        ...
        >
          <div className="flex h-full flex-col items-center justify-center">
            ...
            {listNavbar.map((item, index) => (
              <p
                key={index}
                className="w-full cursor-pointer px-4 py-2 text-lg font-semibold text-black opacity-80 transition duration-300 ease-in-out hover:bg-(--bg-primary-cray) hover:opacity-100"
